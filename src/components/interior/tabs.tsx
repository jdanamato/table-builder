...
export function Tabs({
  ...
  return (
    <div
      className={`w-full overflow-hidden rounded-xl border bg-card ${className}`}
    >
      <div
        {...tabs.tabListProps}
        ref={rowRef}
        aria-label={label}
        className="relative flex w-full gap-1 border-b bg-muted/40 px-1 pt-1"
      >
        <motion.span
          layout
          aria-hidden
        ...
        >
          <motion.span
            ...
            transition={reduced ? { duration: 0 } : INDICATOR}
            className="absolute inset-0 border border-b-0"
          />
        </motion.span>
        ...
        {items.map((item, index) => {
          ...
          return (
            <button
              ...
              className={`relative flex h-8 shrink-0 items-center justify-center rounded-t-[8px] px-3.5 text-[12.5px] outline-none transition-colors duration-150 after:pointer-events-none after:absolute after:inset-0 after:rounded-t-[8px] after:content-[''] focus-visible:after:shadow-[inset_0_0_0_1px_var(--ring)] ${
                item.disabled
      ...
      {renderPanel ? (
        <motion.div
          ...
          className={`rounded-[11px] text-sm leading-relaxed text-foreground outline-none focus-visible:shadow-[inset_0_0_0_1px_var(--ring)] ${panelClassName}`}
        >
